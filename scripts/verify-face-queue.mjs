...
function server(owner, faceResponse = (body) => response(receipt(body))) {
  ...
  globalThis.fetch = async (url, init) => {
    ...
    if (url === '/api/capture') return response({ status: 'recorded', itemId: body.captureId, ...(body.original ? { original: body.original } : {}) })
...
try {
  ...
  await check('a source conflict requires review, then retries original bytes and crop with fresh capture and operation IDs', async () => {
    ...
    assert.equal(rebased.originalBackup, undefined)
    assert.notEqual(rebased.faceTarget.operationId, item.faceTarget.operationId)
    assert.deepEqual(rebased.faceTarget.base, faceBaseline(current))
    ...
    globalThis.fetch = async (url, init) => {
      ...
      if (url === '/api/capture') {
        ...
        return response(body.action === 'status' ? { status: 'missing', original: null } : { status: 'recorded', itemId: body.captureId, ...(body.original ? { original: body.original } : {}) })
      }
      assert.equal(url, '/api/capture/face')
    ...
    assert.equal(await uploads[0].bytes.text(), 'untouched camera bytes')
  ...
  await check('a new face uploads prepared bytes and retains its untouched original after confirmation', async () => {
    ...
    globalThis.fetch = async (url, init) => {
      ...
      if (url === '/api/capture') return response(body.action === 'status' ? { status: 'missing', original: null } : { status: 'recorded', itemId: body.captureId, ...(body.original ? { original: body.original } : {}) })
      assert.equal(url, '/api/capture/face')
