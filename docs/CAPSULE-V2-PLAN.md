...
| 8 | **Board** ◐ | Pan/zoom, drag+persist, clusters w/ drop-to-tag, SCATTER/TIDY, zoom chip, unfiled chip | ✅ core verified (drag persists, drop applies tags, tidy idempotent — `db:verify` 28 checks). **2026-08-01:** filter rail (PEOPLE/PLACES/YEARS/KIND, client-side over the board payload) and the phone sheet (tap a cutout on a coarse pointer → 296px sheet, Peel & move / Open) shipped; a tap no longer writes a z bump on touch. **2026-08-01:** hover card shipped (drag lift `scale(1.06)` composed into the inline transform, card flips left at the canvas edge). **Phase 8 has no deferrals left.** |
