...
| 6 | **Capture pipeline** ◐ | 6a ingest+EXIF ✅ → 6b manual corners ◐ (crop path done, drag UI pending) → 6c sharp derivatives ✅ → 6d Claude extraction ✅ built, unverified (no `ANTHROPIC_API_KEY`) | `db:verify:p6` — 13 checks, upload → private original → derive → public cutout → object with recto face, idempotent on retry |
