...
| 1 | **Platform wiring** | New Vercel project (`vercel link`); `vercel integration add neon`; Blob store; Clerk app + `proxy.ts` + `<ClerkProvider>` + webhook | Sign in on a preview deploy; a `users` row appears via webhook |
| 2 | **Schema + data layer** | Drizzle schema, first migration, `pg_trgm` + GIN indexes, `owner_counters` + lot allocation, `src/server/**` query fns, seed script (~40 objects, 6 people, 5 places) | `drizzle-kit check` clean; seed runs; lot numbers are gapless under concurrent inserts |
| 3 | **Design system** | Tokens for all three surfaces; fonts; `<Cutout>` + `<TiltSurface>` + `<FieldRows>` + `<MonoLabel>` + `<Chip>` + `<RetentionToggle>` + `<Inspector>` + `<SheetPhone>` + `<ShelfRule>` + `<GrainSurface>` + `<StickerDeck>` + `<ScanFrame>`; all 7 silhouettes | A `/design` gallery page rendering every primitive × every surface × every state, side by side with the doc |
| 4 | **Ledger** | `/timeline` full desktop: rail, toolbar, year/month grouping, cutout runs, live inspector | Screenshot diff against `1a`; seeded data groups correctly incl. unknown dates |
| 5 | **Object detail** | `/o/[lot]` phone + desktop; faces (recto/verso/detail); edit via Server Actions; retention toggle; tags | Edit round-trips; `28 more from Dad / 2003 — 2024` computes |
| 6 | **Capture pipeline** | 6a ingest+EXIF → 6b manual corners then auto-detect → 6c sharp derivatives → 6d Claude extraction w/ confidence | End-to-end: photograph a real ticket stub → filed object with a correct cutout and ≥3 pre-filled fields |
| 7 | **Queue** | `/queue` batch tagging; `TAP WHAT'S TRUE`; deck; skip/file; unfiled count everywhere incl. app badge | File 10 seeded unfiled items in under 90 seconds |
| 8 | **Board** | Pan/zoom, drag+persist, clusters, drop-to-file w/ hover card, SCATTER/TIDY/CLUSTER BY, filters, zoom chip, phone sheet | Drag survives reload; TIDY is idempotent; 60fps dragging with 412 objects |
| 9 | **Cabinet** | Shelves + shelf light, lot numbering, gold system, right panel, phone lot view w/ verso flip, dark accession scan chrome | Screenshot diff against `1c`; the shelf-light rule matches |
| 10 | **Index screens** | People / Places / Occasions / search (trgm) / `CATALOGUE` table view | Search finds by lot no., person, place, and free text |
| 11 | **PWA hardening** | Manifest + share target + shortcuts + offline queue + Serwist strategies + install affordance + iOS splash + badging + push | Airplane-mode: capture 5 objects, re-connect, all 5 sync. Share sheet → Capsule works on a real iPhone. Lighthouse PWA + a11y ≥ 95 |
