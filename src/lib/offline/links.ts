...
export function linkChoices(snapshot: SyncSnapshot, field: LinkField): LinkReference[] {
  const rows = field === 'atPlace' ? snapshot.places : field === 'onOccasion' ? snapshot.occasions : field === 'tagged' ? snapshot.tags : field === 'inCollections' ? snapshot.collections.filter(row => row.kind !== 'smart' && !row.creationRejected && (!row.pendingCreation || row.localOnly)) : snapshot.people
  return rows.map(row => ({ id: row.id, name: String(row.name), ...(row.localOnly && !row.pendingCreation ? { create: true as const } : {}) }))
}
...
export function withObjectLinks(snapshot: SyncSnapshot): SyncSnapshot {
  ...
  const collections = new Map(snapshot.collections.filter(row => row.kind !== 'smart').map(row => [row.id, { id: row.id, name: String(row.name), ...(row.localOnly && !row.pendingCreation ? { create: true as const } : {}) }]))
