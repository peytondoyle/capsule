...
const fields = ['title', 'kind', 'receivedAt', 'retention', 'retainedLocation', 'material', 'widthMm', 'heightMm', 'story'] as const

function valuesFor(record: ArchiveRecord): Record<EditField, string> {
  return Object.fromEntries([...fields, 'receivedPrecision'].map((key) => [key,
...
function canonical(values: Record<EditField, string>) {
  return Object.fromEntries(Object.entries(values).map(([field, value]) => [field,
    field === 'title' ? value.trim()
      : field === 'widthMm' || field === 'heightMm' ? value === '' ? null : Number(value)
        : value || null,
  ]))
}
