...
import { reviewShelfCreation, shelfCreations, shelfEdits, shelfOrders, shelfMembershipEdits } from '@/lib/offline/shelves'
...
export function OfflineShelves({ snapshot, operations, onRename, onCreate, onDiscardCreation, onRemove, onOrder, onClose }: {
  ...
  onDiscardCreation: (operationId: string, token: string) => Promise<void>
  onRemove: (id: string) => void
  onOrder: () => void
  onClose: () => void
}) {
  const [name, setName] = useState(''), [saving, setSaving] = useState(false), [error, setError] = useState('')
  async function save(operationId?: string, token?: string) {
    setSaving(true); setError('')
    try { if (operationId) await onDiscardCreation(operationId, token!); else { await onCreate(name); setName('') } }
  ...
  return <main data-surface="ledger" className="safe-t safe-b min-h-dvh bg-bg text-ink"><div className="mx-auto max-w-[620px] px-6 pb-10">
    ...
    {creations.filter(entry => entry.response?.outcome === 'rejected').map(entry => {
      const review = reviewShelfCreation(operations, entry.operationId)!
      const affectedIds = [...new Set(review.affected.flatMap(item => item.mutation.type === 'object.patch' ? [item.mutation.patch.id] : []))]
      return <section key={entry.operationId} className="mt-4 border-b border-hair pb-3"><p className="break-words text-[14px]">The archive could not create {entry.mutation.type === 'collection.create' ? entry.mutation.values.name : 'this shelf'}. {affectedIds.length ? 'Cancel this shelf reference to keep syncing. Other object details and shelf selections stay saved.' : 'Save a copy of the name before discarding it.'}</p>
        {affectedIds.length ? <ul className="mt-2 text-[13px]">{affectedIds.map(id => <li key={id}>{String(snapshot.records.find(row => row.id === id)?.title ?? id)}</li>)}</ul> : null}
        <a className={`${buttonClass} inline-flex items-center`} href={`data:application/json;charset=utf-8,${encodeURIComponent(JSON.stringify({ creation: entry, affected: review.affected }, null, 2))}`} download={`capsule-shelf-${entry.operationId}.json`}>SAVE NEW SHELF DETAILS</a><button type="button" className={buttonClass} disabled={saving || !review.safe} onClick={() => { void save(entry.operationId, review.token) }}>{affectedIds.length ? 'CANCEL FAILED SHELF REFERENCE' : 'DISCARD REJECTED SHELF'}</button>
        {!review.safe ? <p role="alert" className="mt-2 text-[13px]">These dependent edits cannot be safely changed here. Save the pending details for recovery.</p> : null}
      </section>
    })}
    ...
    <ul className="mt-6">{shelves.map(row => {
      ...
      return <li key={row.id} className="border-b border-hair py-4"><h2 className="break-words text-[16px] font-medium">{String(row.name)}</h2><p className="mn mt-2 text-[9px] tracking-[0.1em] text-mute-2">{count} {count === 1 ? 'OBJECT' : 'OBJECTS'}</p>{shelfEdits(operations, row.id).length ? <p className="mn mt-2 text-[9px] tracking-[0.1em] text-accent">{reviews.includes(row.id) ? 'NAME NEEDS REVIEW' : 'NAME SAVED ON DEVICE'}</p> : null}
        ...
        {row.pendingCreation ? <p className="mt-2 text-[13px] text-mute-2">Objects can be added on this device. Sync this new shelf before renaming it.</p> : row.localOnly ? <p className="mt-2 text-[13px] text-mute-2">Sync this new shelf before renaming it.</p> : null}
      </li>
    })}</ul>
